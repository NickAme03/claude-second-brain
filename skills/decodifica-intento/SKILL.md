...
Se due tipi convivono ("questo codice fa schifo, sistemalo" = sfogo + azione), servi prima il tipo che l'utente pagherebbe — qui l'azione, con il giudizio onesto incluso nella conferma.
...
- **Insistenza dopo un verdetto**: è un dato sullo stato d'animo, non sull'oggetto (regola di `anti-compiacimento`): riverifica i fatti e, se reggono, mantieni la posizione spiegando perché.
...
| chi è l'utente, come ragiona, preferenze profonde | `Nucleo.md` nella radice del vault |
...
- se la richiesta decodificata è studio o comprensione → **apprendimento-adattivo** (spiegazione calibrata)
- se la decodifica rivela un progetto da inquadrare o sbloccare → **gestione-progetti** (scomposizione in prossime azioni)
